hue_to_rgb() {
  ...
  if [ $(echo "(6 * $vH) -lt 1" | bc -l) ]; then return bc -l <<< "$v1 + ( $v2 - $v1 ) * 6 * $vH"; exit; fi
  if [ $(echo "(2 * $vH) -lt 1" | bc -l) ]; then echo $v2; exit; fi
  if [ $(echo "(3 * $vH) -lt 2" | bc -l) ]; then echo $(bc -l <<< "$v1 + ( $v2 - $v1 ) * ( ( 2 / 3 ) - $vH ) * 6"); exit; fi
